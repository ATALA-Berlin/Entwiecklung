            ...
            <header class="product--header">
                {block name='frontend_detail_index_header_inner'}
                    <div class="product--info">
						...
						{*****Artikle Name erweitenr*******}
							{$sArticle.articleName} {$sArticle.additionaltext|escape}
						</h1>
                            {/block}
            ...
            {block name='frontend_detail_index_buy_container'}
				...
				<ul class="product--base-info list--unstyled">
					...
					<li class="base-info--entry entry--sku">
						{* Supplier name *}
						{if $sArticle.supplierName and $sArticle.supplierName!="Atala"}
						{block name='frontend_detail_index_supplier'}
						<strong class="entry--label">
							{se name="DetailFromNew"}Hersteller:{/se}
						</strong>						
						{* Product SKU - Content *}
						{block name='frontend_detail_data_ordernumber_supplierName'}
					...
					{* Herstellernummer gtin13 *}
					{if $sArticle.suppliernumber}
					<li class="base-info--entry entry--sku">
						{block name='frontend_detail_index_suppliernummer'}
						<strong class="entry--label">
							{se name="Herstellernummer"}Hersteller-Artikelnummer:{/se}
						</strong>				
						<meta itemprop="mpn" content="{$sArticle.suppliernumber}" />
						<meta itemprop="gtin13" content="{$sArticle.ean|escape}" />
						<span class="entry--content">
				...
				{******fläschenberechener_fliesen********}
				{block name="fläschenberechener_fliesen"}
			{if $sArticle.attr8 =="Fliese"}
			<div class="rechner" style="float:right;margin-top:-10px;">
				<p>Mengenberechnung</p>
				<div>
					Fläche: <input id="testinput" onkeyup="myFunct()" value="" size="5" title="Fläche in Qm"  maxlength="12" > m<sup>2</sup> =
					<input id="testoutput" onkeyup="myFunction()" value="" size="5" title="Anzahl Karton(s)"  maxlength="12" > Karton(s) = 
					<input id="test" value="" size="5" title="Anzahl QM"  maxlength="12" > Quadratmeter
					
				</div>
				<script>
					function myFunct() {
						
						var a= document.getElementById("testinput").value.replace( ",","." );
						var b = document.getElementById("testoutput");
						var c = document.getElementById("test");
						var temp=Math.ceil( parseFloat(a) /({$sArticle.attr2}));
						...
						if (isNaN(temp)){
							...
							b.value=Math.ceil(parseFloat(document.getElementById("testinput").value.replace( ",","." ))/({$sArticle.attr2}));
							c.value = Math.round( (Math.ceil(parseFloat(document.getElementById("testinput").value.replace( ",","." ))/({$sArticle.attr2}))*({$sArticle.attr2}))*100)/100;
						}
					}
					...
					function myFunction() {
						...
						z.value = Math.round( (parseFloat(document.getElementById("testoutput").value.replace( "","" ))*({$sArticle.attr2}))*100)/100;
						x.value ="";
						if (isNaN(z.value)){
							z.value="";
						}
