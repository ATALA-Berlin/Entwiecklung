...
{block name='frontend_index_content'}
	<div class="content listing--content">
		...
		{block name="frontend_listing_index_banner"}
			...
			{/if}
		{/block}
		{if {$sCategoryCurrent}== 3 }

		{block name="frontend_listing_text_max"}
    <div class="vendor--info panel has--border">

        {* Vendor headline *}
        {block name="frontend_listing_list_filter_supplier_headline_max"}
            <h1 class="panel--title is--underline">
                {s name='ListingInfoFilterSupplierMax'}Produkte von {/s} {$manufacturer->getName()|escapeHtml}
            </h1>
        {/block}

        {block name="frontend_listing_list_filter_supplier_content_max"}
		...
		{if $manufacturer->getAttributes() }
		{$attr.core->get("kurzbeschreibung")}
		{/if}
		</div>
		{/if}
