...
{function name=customPages level=0}
    <ul class="shop-sites--navigation sidebar--navigation navigation--list{if !$level} is--drop-down{/if} is--level{$level}" role="menu">
        {block name='frontend_index_left_menu_before'}
...
{if $sMenu.gUeberUns && $sCustomPage.grouping|strstr:"gUeberUns" }
    {block name="frontend_index_left_menu_container"}
        <div class="shop-sites--container is--rounded">
            {block name='frontend_index_left_menu_headline'}
                <div class="shop-sites--headline navigation--headline">
                    {s namespace='frontend/index/menu_left' name="MenuLeftHeadingUeberUns"}{/s}
                </div>
            {/block}
            {call name=customPages customPages=$sMenu.gUeberUns}
        </div>
    {/block}
{elseif $sMenu.gFilialen && $sCustomPage.grouping|strstr:"gFilialen"}
    {block name="frontend_index_left_menu_container"}
        <div class="shop-sites--container is--rounded">
            {block name='frontend_index_left_menu_headline'}
                <div class="shop-sites--headline navigation--headline">
                    {s namespace='frontend/index/menu_left' name="MenuLeftHeadingFilialen"}{/s}
                </div>
            {/block}
            {call name=customPages customPages=$sMenu.gFilialen}
        </div>
    {/block}
{elseif $sMenu.gService && $sCustomPage.grouping|strstr:"gService"}
    {block name="frontend_index_left_menu_container"}
        <div class="shop-sites--container is--rounded">
            {block name='frontend_index_left_menu_headline'}
                <div class="shop-sites--headline navigation--headline">
            ...
            {/block}
            {call name=customPages customPages=$sMenu.gService}
        </div>
    {/block}
{elseif $sMenu.gLeft}
    {block name="frontend_index_left_menu_container"}
